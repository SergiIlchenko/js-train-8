...
function createArray(...elements) {
  if (elements.length > 0) {
    const newArry = Array.of(...elements);
    return newArry;
  } else {
    console.log("Помилка: Не передано жлдного елеманту.");
    return [];
  }
...
function copyAndSwapElements(arr, target, start, end) {
  if (!arr.includes(target, start, end)) {
